...
```
В задачах на оценку от 6 и до 8 может произойти проблема с тем, что в случае некорректного исполнения придется перейти в директорию `/tmp` и удалить там каналы `fifo1` и `fifo2`, после этого удастся дальше работать.
